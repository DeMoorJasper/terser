...
function minify(files, options) {
    ...
    try {
        ...
        if (!HOP(options.output, "code") || options.output.code) {
            ...
            if (options.sourceMap) {
                // The `.map` property is lazy to allow callers to opt-in to map stringification by accessing the value
                Object.defineProperty(result, "map", {
                    configurable: true,
                    enumerable: true,
                    get() {
                        if (options.sourceMap.asRawMappings) {
                            return (this.map = options.output.source_map.get()._mappings._array);
                        } else if (options.sourceMap.asObject) {
                            return (this.map = options.output.source_map.get().toJSON());
                        } else {
                            return (this.map = options.output.source_map.toString());
                        }
                    },
                    set(value) {
